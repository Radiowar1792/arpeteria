...
## Orthographes utilisées par œuvre (fait le 2026-07-05)

Demande du conservateur : une œuvre peut mélanger ou illustrer plusieurs orthographes du
savoyard/arpitan (ex : ORB, graphie de Conflans, graphie de Savoie…). Ajout d'un champ
`orthographes` (tags libres, comme `mots_cles`) sur la collection `oeuvres`, affiché en tags juste
sous le titre de la fiche œuvre. Le site permet maintenant de naviguer par orthographe :

- Page `/orthographe/<slug>` listant toutes les œuvres publiées pour une orthographe donnée
  (même principe que `/mot-cle/<slug>`).
- Filtre par orthographe sur les pages de catégorie (`/archives/<periode>`,
  `/contemporain/<discipline>`), combiné avec le filtre par langue existant (nouveau composant
  `FiltresOeuvres.astro`, qui remplace `FiltreLangues.astro`).
- Section « Par orthographe » sur `/collection` (la page "La collection en chiffres").
- Facette « Orthographe » dans la recherche Pagefind (`/recherche`), au même titre que
  Section/Type/Langue/Catégorie.

**Pour appliquer ce nouveau champ sur le LXC existant** : ajouter manuellement le champ dans
Directus (Paramètres → Modèles de données → oeuvres → Créer un champ → `orthographes`, type
JSON, interface "Tags") — ou relancer `npm run bootstrap` (idempotent, ne touche pas au contenu
déjà saisi). Ensuite reconstruire le site : `CACHEBUST=$(date +%s) docker compose up -d --build
web` (regénère aussi l'index de recherche Pagefind avec la nouvelle facette).
